...
const COMPLETED_MODULES_STORAGE_KEY = "flowdex:expert-investment:completed-modules:v1"
const COURSE_SLUG = "expert-investment"
// URLs de Cal.com (cuenta augusto-holman-flwjaq) para las clases en vivo de
// Expert Investment.
const CLASS_SCHEDULING_MILESTONES = {
  1: { requiredModules: [1], label: "Agendar clase 1" },
        ...
        <div className="flex flex-wrap gap-px bg-[#2A2A2A]">
          {block.items.map((item, i) => (
            <div
              ...
              className="flex min-h-[128px] flex-col items-center justify-center gap-2 bg-[#0F0F0F] px-4 py-3 text-center sm:px-4 sm:py-3.5"
            >
              <p className={`text-2xl font-bold leading-tight ${colors.number}`}>{item.label}</p>
              <p className="mx-auto max-w-[30ch] text-xs leading-[1.45] text-[#888888]">{item.text}</p>
            </div>
          ))}
        </div>
      </div>
...
function ModuleSection({
  ...
  const sectionId = `ei-m${moduleNumber}-s${sectionIndex + 1}`
...
export function ExpertInvestmentContent() {
  ...
  const realModules = useMemo(() => expertInvestmentContent.filter((mod) => mod.number > 0), [])
  ...
  useEffect(() => {
    ...
    async function hydrateProgress() {
      ...
      if (targetModule) {
        const resumeIndex = expertInvestmentContent.findIndex((module) => module.number === targetModule.number)
        if (resumeIndex >= 0) {
  ...
  useEffect(() => {
    ...
    const current = expertInvestmentContent[page * MODULES_PER_PAGE]
    ...
    if (intent === "top") {
      ...
      document.getElementById("ei-modulo-indice")?.scrollIntoView({ behavior: "auto", block: "start" })
    }
    // "stay": no tocamos el scroll.
  }, [page, isProgressReady])

  const totalPages = Math.ceil(expertInvestmentContent.length / MODULES_PER_PAGE)
  ...
  const visibleModules = expertInvestmentContent.slice(
    ...
    <div className="mt-10">
      ...
      <div className="mb-8 overflow-hidden rounded-2xl border border-[#1E1E1E] bg-[#101010]">
        <div className="grid gap-4 p-5 sm:grid-cols-[1.2fr_0.8fr] sm:p-6">
          <div>
            <p className={`text-[11px] uppercase tracking-[0.2em] ${currentModuleColors.number}`}>Expert Investment</p>
          ...
          <div className="relative min-h-[160px] overflow-hidden rounded-xl border border-[#2A2A2A] bg-[#0C0C0C]">
            <Image
              src="/flowdex expert investment 1x1.png"
              alt={`Módulo ${currentModule?.number ?? 1} - Expert Investment`}
              fill
      ...
      <div id="ei-modulo-indice" className="mb-8 scroll-mt-24 rounded-xl border border-[#1E1E1E] bg-[#0F0F0F] p-4 sm:p-5">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          {currentModule.sections.map((section, i) => (
            <a
              key={`${section.title}-${i}`}
              href={`#ei-m${currentModule.number}-s${i + 1}`}
        ...
        <div className="text-center">
          <ModulePills
            ...
            onSelect={(moduleNumber) => {
              const idx = expertInvestmentContent.findIndex((m) => m.number === moduleNumber)
      ...
      <div className="space-y-14">
        {visibleModules.map((mod) => {
          ...
          const isExpertFinalModule = mod.number === 4
              ...
              {isExpertFinalModule && (
                <div className="mt-4 rounded-xl border border-[#7DD4C0]/25 bg-gradient-to-r from-[#5BB8D4]/10 to-[#7DD4C0]/10 p-4 sm:p-5">
                  ...
                  <p className="mt-2 text-sm text-[#D9E5E2]">
                    Si terminaste Expert Investment, el siguiente nivel para consolidar criterio y proceso de inversión es Inner Circle.
                  </p>
