...
import { innerCircleTradingContent, type Block, type Module } from "@/lib/courses/inner-circle-trading-content"
...
const COMPLETED_MODULES_STORAGE_KEY = "flowdex:inner-circle-trading:completed-modules:v1"
const COURSE_SLUG = "inner-circle-trading"
...
function RenderBlock({ block, colors }: { block: Block; colors: (typeof moduleColors)["teal"] }) {
  if (block.type === "exercise") {
    return (
      <div className="rounded-2xl border border-[#2A2A2A] bg-[#121212] px-5 py-5 sm:px-6">
        <div className="mb-4 flex items-center gap-3">
          <span className={`inline-flex h-8 min-w-8 items-center justify-center rounded-full border border-[#2A2A2A] px-2 text-[11px] font-semibold uppercase tracking-[0.16em] ${colors.accentText}`}>
            Ejercicio
          </span>
          <h4 className="text-sm font-semibold uppercase tracking-[0.14em] text-white sm:text-base">
            {block.title}
          </h4>
        </div>
        ...
        <div className="space-y-4">
          {block.blocks.map((nestedBlock, index) => (
            <RenderBlock key={`${block.title}-${index}`} block={nestedBlock} colors={colors} />
          ))}
        </div>
      </div>
    )
          ...
          <thead>
            <tr className="border-b border-[#2A2A2A] bg-[#1A1A1A]">
              {block.headers.map((header, index) => (
                <th
                  key={`${header}-${index}`}
                  className={`px-4 py-2.5 text-left text-[11px] font-semibold uppercase tracking-[0.15em] ${colors.accentText}`}
                >
                  {header}
                </th>
              ))}
              ...
              <tr key={`row-${rowIndex}`} className="border-b border-[#1E1E1E] last:border-0">
                {row.map((cell, cellIndex) => (
                  <td
                    ...
                    className={`px-4 py-3 align-top text-xs leading-relaxed ${
                      cellIndex === 0 ? `font-semibold ${colors.accentText}` : "text-[#CCCCCC]"
                    }`}
                  >
                    {cell}
                  </td>
...
function ModuleSection({
  ...
  const sectionId = `ict-m${moduleNumber}-s${sectionIndex + 1}`
...
export function InnerCircleTradingContent() {
  ...
  const modules = useMemo(() => innerCircleTradingContent, [])
  ...
  useEffect(() => {
    ...
    async function hydrateProgress() {
      const { storageKey, modules: storedModules } = await loadCompletedModules(
  ...
  useEffect(() => {
    ...
    if (intent === "top") {
      ...
      document.getElementById("ict-modulo-indice")?.scrollIntoView({ behavior: "auto", block: "start" })
    }
  ...
  return (
    <div className="mt-10">
      <CourseBreadcrumb
        courseName="Inner Circle · Trading"
      ...
      <div className="mb-8 overflow-hidden rounded-2xl border border-[#1E1E1E] bg-[#101010]">
        <div className="grid gap-4 p-5 sm:grid-cols-[1.25fr_0.75fr] sm:p-6">
          <div>
            <p className={`text-[11px] uppercase tracking-[0.2em] ${currentModuleColors.number}`}>Inner Circle · Trading</p>
            <h2 className="mt-2 type-display-sm text-white">
              Módulo {currentModule?.number}
            </h2>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-[#B9B9B9]">{currentModule?.subtitle}</p>
            <div className="mt-4 inline-flex items-center gap-2 rounded-lg border border-[#7DD4C0]/35 bg-[#7DD4C0]/10 px-3 py-2 text-xs text-[#CCF0E8]">
              Diseñado para operar futuros con criterio profesional: setup, ejecución, contexto y riesgo.
            </div>
          </div>

          <div className="rounded-xl border border-[#2A2A2A] bg-[linear-gradient(180deg,#0F201D,#0B0F11)] p-5">
            <p className="text-[11px] uppercase tracking-[0.18em] text-[#7DD4C0]">Disciplina trading</p>
            <p className="mt-3 text-sm leading-relaxed text-[#D3E5EA]">
              Cinco módulos secuenciales para operar ORB en NY con estructura completa: setup, ejecución intradía, contexto macro,
              noticias y control profesional de riesgo.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-8 grid gap-3 sm:grid-cols-3">
        <div className="rounded-xl border border-[#2A2A2A] bg-[#101010] p-4">
          ...
          <p className="mt-1 text-sm text-[#D4D4D4]">Setup ORB adaptativo + filtros de contexto + ejecución estructurada.</p>
        </div>
        <div className="rounded-xl border border-[#2A2A2A] bg-[#101010] p-4">
          <p className="text-[10px] uppercase tracking-[0.2em] text-[#5BB8D4]">Ritmo</p>
          <p className="mt-1 text-sm text-[#D4D4D4]">Módulos secuenciales. Primero dominar estructura, luego velocidad.</p>
        </div>
        <div className="rounded-xl border border-[#2A2A2A] bg-[#101010] p-4">
          <p className="text-[10px] uppercase tracking-[0.2em] text-[#5BB8D4]">Objetivo</p>
          <p className="mt-1 text-sm text-[#D4D4D4]">Terminar con un protocolo operable para cuenta propia y cuentas de fondeo.</p>
        </div>
      </div>

      <div id="ict-modulo-indice" className="mb-8 scroll-mt-24 rounded-xl border border-[#1E1E1E] bg-[#0F0F0F] p-4 sm:p-5">
        <p className="text-[11px] uppercase tracking-[0.2em] text-[#7DD4C0]">Índice rápido del módulo</p>
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          {currentModule.sections.map((section, index) => (
            <a
              key={`${section.title}-${index}`}
              href={`#ict-m${currentModule.number}-s${index + 1}`}
        ...
        {Array.from({ length: totalPages }).map((_, index) => (
          <button
            ...
            className={`h-1.5 rounded-full transition-all duration-300 ${
              index === page ? "w-8 bg-[#7DD4C0]" : "w-3 bg-[#2A2A2A] hover:bg-[#444444]"
            }`}
          ...
          return (
            <div key={`${module.number}-${module.title}`}>
              <div className="mb-6 flex items-start gap-4">
                <div
                  className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border ${colors.badge} font-mono text-sm font-bold`}
                >
              ...
              {isFinalModule ? (
                <div className="mt-4 rounded-xl border border-[#7DD4C0]/25 bg-gradient-to-r from-[#5BB8D4]/10 to-[#7DD4C0]/10 p-4 sm:p-5">
                  ...
                  <p className="mt-2 text-sm text-[#D9E5E2]">
                    Al terminar esta disciplina, volvés al panel principal de Inner Circle para integrar Obra Maestra, Inversiones y la
                    ejecución semanal completa.
                  </p>
