...
const COMPLETED_MODULES_STORAGE_KEY = "flowdex:kickstart-trading:completed-modules:v2"
const COURSE_SLUG = "kickstart-trading"
// URLs de Cal.com (cuenta franco-escudero-5idua4) para las clases en vivo de
// Kickstart Trading. Cada milestone desbloquea la URL correspondiente cuando
// se completan los módulos requeridos.
...
function ModuleSection({
  ...
  const sectionId = `kt-m${moduleNumber}-s${sectionIndex + 1}`
...
export function KickstartTradingContent() {
  const [page, setPage] = useState(0)
  // Capturamos el ?m=N de la URL una sola vez, durante el primer render, antes
  // de que cualquier efecto lo sobreescriba. Permite retomar el módulo exacto.
  ...
  const sortedModules = useMemo(
    () => [...kickstartTradingContent].sort((a, b) => a.number - b.number),
    []
  )
  const realModules = useMemo(() => sortedModules.filter((mod) => mod.number > 0), [sortedModules])
  ...
  useEffect(() => {
    ...
    async function hydrateProgress() {
      ...
      // Si la URL trae ?m=N válido, ese módulo manda sobre el "primer incompleto".
      ...
      if (targetModule) {
        const resumeIndex = sortedModules.findIndex((module) => module.number === targetModule.number)
        if (resumeIndex >= 0) {
  ...
  }, [realModules, sortedModules, totalRealModules, initialModuleParam])
  ...
  useEffect(() => {
    // Esperamos a que el progreso esté hidratado para no pisar el ?m=N inicial
    // de la URL con el page=0 del primer render.
    if (!isProgressReady) {
      return
    }
    const current = sortedModules[page * MODULES_PER_PAGE]
    ...
    if (intent === "top") {
      ...
      document.getElementById("kt-modulo-indice")?.scrollIntoView({ behavior: "auto", block: "start" })
    }
    // "stay": no tocamos el scroll.
  }, [page, isProgressReady, sortedModules])

  const totalPages = Math.ceil(sortedModules.length / MODULES_PER_PAGE)
  ...
  const visibleModules = sortedModules.slice(
  ...
  //   el alumno ya está arriba y un salto sería molesto).
  // - "index": navegación desde los controles de abajo (lo llevamos al índice
  //   rápido del nuevo módulo para que arranque a leer desde ahí).
    ...
    <div className="mt-10">
      ...
      <div className="mb-8 overflow-hidden rounded-2xl border border-[#1E1E1E] bg-[#101010]">
        <div className="grid gap-4 p-5 sm:grid-cols-[1.2fr_0.8fr] sm:p-6">
          <div>
            <p className={`text-[11px] uppercase tracking-[0.2em] ${currentModuleColors.number}`}>Kickstart Trading</p>
          ...
          <div className="relative min-h-[160px] overflow-hidden rounded-xl border border-[#2A2A2A] bg-[#0C0C0C]">
            <Image
              src="/flowdex kickstart trading 1x1.png"
              alt={`Módulo ${currentModule?.number ?? 1} - Kickstart Trading`}
              fill
              sizes="(max-width: 640px) 100vw, 360px"
              className="object-cover object-center opacity-80 invert"
            />
      ...
      <div className="mb-8 grid gap-3 sm:grid-cols-3">
        <div className="rounded-xl border border-[#2A2A2A] bg-[#101010] p-4">
          ...
          <p className="mt-1 text-sm text-[#D4D4D4]">Aplica cada modulo con registro operativo y revision posterior.</p>
        </div>
        <div className="rounded-xl border border-[#2A2A2A] bg-[#101010] p-4">
        ...
        <div className="rounded-xl border border-[#2A2A2A] bg-[#101010] p-4">
          ...
          <p className="mt-1 text-sm text-[#D4D4D4]">Construir consistencia bajo presion con disciplina de proceso.</p>
        </div>
      </div>

      <div id="kt-modulo-indice" className="mb-8 scroll-mt-24 rounded-xl border border-[#1E1E1E] bg-[#0F0F0F] p-4 sm:p-5">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          {visibleModules[0].sections.map((section, i) => (
            <a
              key={`${section.title}-${i}`}
              href={`#kt-m${visibleModules[0].number}-s${i + 1}`}
        ...
        <div className="text-center">
          <ModulePills
            ...
            onSelect={(moduleNumber) => {
              const idx = sortedModules.findIndex((m) => m.number === moduleNumber)
